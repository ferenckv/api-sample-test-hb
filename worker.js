...
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    const contactsToGet = _.map(
      _.uniqBy(contactAssociationsResults, "to.0.id"),
      "to.0.id"
    );

    const contactsResponse = await hubspotClient.crm.contacts.batchApi.read(
      {
        inputs: contactsToGet.map((contactId) => ({ id: contactId })),
      },
      false
    );

    const contactEmails = _.reduce(contactsResponse.results, (acc, item) => {
      acc[item.id] = item.properties.email;
      return acc;
    }, {});
      ...
      contactAssociationsResults
        .map((a) => {
          if (a.from) {
            ...
            return [a.from.id, contactEmails[a.to[0].id]];
          } else return false;
        })
        .filter((x) => x)
    );
